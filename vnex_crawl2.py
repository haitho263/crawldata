...
def saveArticles(base_url,path):
    ...
    for index, item in enumerate(data[::-1]):
        ...
        try:
            ...
            db.rollback()
            try:
                query = "ALTER TABLE articles AUTO_INCREMENT = {}".format(getLastId())
                cursor.execute(query)
                db.commit()
            except:
                pass
...
def getArticlesByCategory(category, offset):
    db = mysql.connector.connect(user=USERNAME, password=PASSWORD,
                                 ...
                                 database=DATABASE)
    cursor = db.cursor()
    query='SELECT * FROM datatrends.articles where  category LIKE "%{}%" ORDER BY time2 DESC LIMIT {} OFFSET {}'.format(category,ARTICLES_PER_LOAD,offset)
    cursor.execute(query)
    dataNews = []
    records = cursor.fetchall()
    for row in records:
...
def getHotNews(offset):
    db = mysql.connector.connect(user=USERNAME, password=PASSWORD,
                                 ...
                                 database=DATABASE)
    cursor = db.cursor()
    query = 'SELECT * FROM datatrends.articles where  is_hot = True ORDER BY time2 DESC LIMIT {} OFFSET {}'.format(ARTICLES_PER_LOAD,offset)
    cursor.execute(query)
    dataNews = []
    records = cursor.fetchall()
    for row in records:
...
# SELECT * FROM datatrends.articles
# where (title LIKE '%viet%') or (description LIKE '%viet%') ORDER BY time2 DESC LIMIT 10 OFFSET 0
def searchArticles(info,offset):
    ...
    query = "SELECT * FROM datatrends.articles where (title LIKE '%{}%') or (description LIKE '%{}%')" \
            " ORDER BY time2 DESC LIMIT {} OFFSET {}".format(info,info,ARTICLES_PER_LOAD,offset)
    cursor.execute(query)
    dataNews = []
    records = cursor.fetchall()
    for row in records:
